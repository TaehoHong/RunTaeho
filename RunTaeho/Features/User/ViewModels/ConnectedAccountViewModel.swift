...
class ConnectedAccountViewModel: ObservableObject {
    ...
    private let service: UserAccountServiceProtocol
    ...
    func loadConnectedAccounts() {
        Task {
            ...
            do {
                let accounts = try await service.getConnectedAccounts()
                
                // 연결 상태 업데이트
                for account in accounts {
                    self.connectionStatus[account.provider] = account.isConnected ? .connected : .none
                }
            } catch {
        ...
        Task {
            do {
                ...
                if isCurrentlyConnected {
                    success = try await service.disconnectAccount(provider: provider)
                } else {
                    success = try await service.connectAccount(provider: provider)
                }
                
                if success {
                    // 연결 상태 업데이트
                    self.connectionStatus[provider] = isCurrentlyConnected ? .none : .connected
                    
                    // 계정 목록 새로고침
                    loadConnectedAccounts()
                } else {
                    self.connectionStatus[provider] = .failed
