...
class GoogleAuthenticationStrategy: AuthenticationStrategy {
    ...
    var providerName: String { "Google" }
    var authType: AuthProvider { .google }
    ...
    func signIn() async throws -> UserAuthData {
        ...
        do {
            ...
            guard let _ = result.user.profile else {
            ...
            // 서버에서 토큰 획득
            return try await AuthenticationService.shared.getToken(provider: authType, code: authCode)
