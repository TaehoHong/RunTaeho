...
class AvatarManagementViewModel: ObservableObject {
    ...
    @Published var currentPreviewItems: [AvatarCategory: AvatarItem] = [:]
    ...
    var selectedCategory: AvatarCategory {
        guard selectedCategoryIndex < categories.count else { return .hair }
        return categories[selectedCategoryIndex].category
    }
    
    var filteredItems: [AvatarItemViewModel] {
        items.filter { item in
            allAvatarItems.first { $0.id == item.id }?.category == selectedCategory
        }
    }
    ...
    var hasChanges: Bool {
        ...
        for category in AvatarCategory.allCases {
            ...
            let originalItem = userStateManager.equippedItems[category]
    ...
    private func setupCategories() {
        categories = AvatarCategory.allCases.map { CategoryViewModel(category: $0) }
    }
    ...
    private func getCurrentEquippedItems() -> [AvatarCategory: AvatarItem] {
    ...
    private func updateEquippedItems() {
        var equipped: [AvatarCategory: AvatarItem] = [:]
